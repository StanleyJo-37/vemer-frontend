...
export function ActivityDetailsDialog({ activity, onClose, onRemoveParticipant }: ActivityDetailsDialogProps) {
  ...
  useEffect(() => {
    const fetchParticipants = async () => {
      ...
      try {
        const response = await PublisherDashboardAPI.getActivityParticipants(activityId)
  ...
  const filteredParticipants = useMemo(() => {
    ...
    return participants.filter((participant: any) => {
      ...
      const matchesStatus = filterStatus === "all" || participant.status?.toLowerCase() === filterStatus.toLowerCase()
  ...
  const handleRemoveParticipant = useCallback(
    (participantId: string, participantName: string) => {
      if (confirm(`Are you sure you want to remove ${participantName} from this activity?`)) {
        ...
        setParticipants((prev: any[]) => prev.filter((p: any) => p.id !== participantId))
      }
    },
    [activity.id, onRemoveParticipant],
  )

  const handleApproveParticipant = useCallback(async (participantId: string, participantName: string) => {
    if (confirm(`Approve ${participantName} for this activity?`)) {
      setParticipants((prev: any[]) =>
        prev.map((p: any) => (p.id === participantId ? { ...p, status: "Confirmed" } : p)),
      )
      const payload = {
  ...
  const handleRejectParticipant = useCallback(async (participantId: string, participantName: string) => {
  if (confirm(`Reject ${participantName}'s registration for this activity?`)) {
    setParticipants((prev: any[]) =>
      prev.map((p: any) => (p.id === participantId ? { ...p, status: "Cancelled" } : p)),
    );
    const payload = {
      activity_id: Number(activityId),
      user_id: Number(participantId),
  ...
  if (confirm(`Are you sure you want to approve all ${pendingParticipants.length} pending participants?`)) {
    // Update UI
    setParticipants((prev: any[]) =>
      prev.map((p: any) =>
        p.status === "Pending"
    ...
    alert(`Successfully approved ${pendingParticipants.length} participants!`);
  }
                  ...
                  <div className="flex-1 flex flex-col space-y-3">
                    ...
                    <div className="space-y-2 text-sm flex-shrink-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>{participants.filter((p: any) => p.status === "Confirmed").length} participants</span>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="h-full overflow-y-auto pr-2">
                    {loading ? (
                      <div className="flex flex-col items-center justify-center h-64">
                        ...
                        <div className="text-gray-600 font-medium">Loading participants...</div>
                      </div>
                    ) : (

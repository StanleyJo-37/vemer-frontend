      ...
      <CardContent className="p-4 sm:p-6">
        <div className="space-y-3 sm:space-y-4">
          {announcements.map((announcement) => (
            <div
              key={announcement.id}
              className={`p-3 sm:p-4 border-l-4 rounded-lg border border-gray-200 hover:border-sky-200 transition-colors ${getPriorityColor(announcement.priority)}`}
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-2 gap-2">
                ...
                <Badge className={`${getTypeColor(announcement.type)} text-xs cursor-default self-start`}>
                  {announcement.type}
                </Badge>
              </div>

...
import { CheckCircle, Users, Star, Award, Calendar, MapPin } from "lucide-react"
...
export function EndEventDialog({ activity, onClose, onEndEvent }: EndEventDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    alert(
      `Event ended! ${attendedCount} participants attended and received ${activity.points} points${activity.badge ? " and a badge" : ""}.`,
    )
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto border-sky-100">
        <DialogHeader className="border-b border-sky-100 pb-4">
          ...
          <DialogDescription>Mark attendance for participants to award points and badges</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card className="border-sky-100">
              <CardHeader>
                <CardTitle className="text-lg text-sky-900">Event Summary</CardTitle>
              </CardHeader>
                  ...
                  <div className="space-y-2">
                    ...
                    {activity.badge && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{activity.badge.name} badge</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="lg:col-span-2">
            <Card className="border-sky-100 h-full">
              <CardHeader>
                <CardTitle className="text-lg text-sky-900">Mark Attendance</CardTitle>
                <p className="text-sm text-gray-600">
                  Check the participants who attended the event to award them points and badges
                </p>
              </CardHeader>
              <CardContent>
                ...
                {(!activity.participants || activity.participants.length === 0) && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600">No one registered for this event</p>
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-sky-100">
          <div className="text-sm text-gray-600">
            {attendedCount > 0 && (
              <span>
                {attendedCount} participant{attendedCount !== 1 ? "s" : ""} will receive {activity.points} points
          ...
          <div className="flex gap-3">
            ...
            >
              {isSubmitting ? "Ending Event..." : `End Event & Award ${attendedCount} Participants`}
            </Button>
          </div>
        </div>
      </DialogContent>
